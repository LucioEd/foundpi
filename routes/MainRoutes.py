...
async def getIndex(request: Request,pa: int = 1,tp: int = 12):
    produtos = ProdutoRepo.obterTodos()
    categorias = CategoriaRepo.obterTodos()
    filtro = False
    produtos = ProdutoRepo.obterPagina(pa, tp)
    totalPaginas = ProdutoRepo.obterQtdePaginas(tp)
    cliente = validar_usuario_logado(request)
    return templates.TemplateResponse("index.html",{ "request": request, "produtos": produtos, "categorias": categorias, 'filtro': filtro ,
...
async def root(request: Request):
    ...
    return RedirectResponse('/fechamento_pedido', status_code=status.HTTP_302_FOUND)
