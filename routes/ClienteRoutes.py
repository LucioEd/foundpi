...
async def root(request: Request):
    token = request.cookies.values().mapping["auth_token"]
    cliente  = ClienteRepo.obterPorToken(token)
    print(cliente)
    return templates.TemplateResponse("cliente_contatos.html", {"request": request, "cliente": cliente,})
...
async def root(request: Request):
    token = request.cookies.values().mapping["auth_token"]
    cliente  = ClienteRepo.obterPorToken(token)
    return templates.TemplateResponse("cliente_dados.html", {"request": request, "cliente": cliente,})
...
async def getAlterarSenha(
    ...
    if cliente:
        return templates.TemplateResponse(
            "cliente_senha.html", {"request": request, "cliente": cliente}
        )
    else:
...
async def postAlterarSenha(
    request: Request,
    cliente: Cliente = Depends(validar_usuario_logado),
    senhaAtual: str = Form(""),
    novaSenha: str = Form(""),
    ...
    if len(erros) == 0:    
        hash_senha_bd = ClienteRepo.obterSenhaDeEmail(cliente.email)
        if hash_senha_bd:
    ...
    ClienteRepo.alterarSenha(cliente.id, hash_nova_senha)
